...
{
    ChipLogProgress(AppServer, "CastingPlayer::StopConnecting() called, while ChipDeviceEventHandler.sUdcInProgress, Trying hack 2 to re-populate mUdcClients cache on TV in order to allow cancel to pass through : %s",
...
{
    ChipLogProgress(AppServer, "CastingPlayer::OnCommissioningSessionEstablishmentError() resetting connection state and clearing Udc status err: %" CHIP_ERROR_FORMAT, err.Format());
    mConnectionState = CASTING_PLAYER_NOT_CONNECTED;
    support::ChipDeviceEventHandler::SetUdcStatus(false);
